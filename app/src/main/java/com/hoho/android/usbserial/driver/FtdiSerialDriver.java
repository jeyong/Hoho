...
import com.jeyong.hoho.MainActivity;
...
//import org.mavlink.qgroundcontrol.MainActivity;
...
public class FtdiSerialDriver extends CommonUsbSerialDriver {
    ...
    public void open() throws IOException {
        ...
        try {
            ftD2xx = D2xxManager.getInstance(MainActivity.m_context);
        } catch (D2xxManager.D2xxException ex) {
            MainActivity.qgcLogDebug("D2xxManager.getInstance threw exception: " + ex.getMessage());
        }

        if (ftD2xx == null) {
            String errMsg = "Unable to retrieve D2xxManager instance.";
            MainActivity.qgcLogWarning(errMsg);
            throw new IOException(errMsg);
        }
        MainActivity.qgcLogDebug("Opened D2xxManager");

        int DevCount = ftD2xx.createDeviceInfoList(MainActivity.m_context);
        MainActivity.qgcLogDebug("Found " + DevCount + " ftdi devices.");
        if (DevCount < 1) {
        ...
        m_ftDev = null;
        try {
            m_ftDev = ftD2xx.openByIndex(MainActivity.m_context, 0);
        } catch (NullPointerException e) {
            MainActivity.qgcLogDebug("ftD2xx.openByIndex exception: " + e.getMessage());
        } finally {
            if (m_ftDev == null) {
                throw new IOException("No FTDI Devices found");
            }
        }
        MainActivity.qgcLogDebug("Opened FTDI device.");
    }
    ...
    public void close() {
        if (m_ftDev != null) {
            try {
                ...
                MainActivity.qgcLogWarning("close exception: " + e.getMessage());
            }
            m_ftDev = null;
        }
    }
    ...
    public int read(byte[] dest, int timeoutMillis) throws IOException {
        ...
        if (bytesAvailable > 0) {
            ...
            try {
                ...
                MainActivity.qgcLogWarning(errorMsg);
    ...
    public int write(byte[] src, int timeoutMillis) throws IOException {
        try {
            ...
            MainActivity.qgcLogWarning("Error writing: " + e.getMessage());
        }
        return 0;
    }
    ...
    private int setBaudRate(int baudRate) throws IOException {
        try {
            ...
            MainActivity.qgcLogWarning("Error setting baud rate: " + e.getMessage());
        }
        return 0;
    }
    ...
    public void setParameters(int baudRate, int dataBits, int stopBits, int parity) throws IOException {
        ...
        try {
            ...
            MainActivity.qgcLogWarning("Error setDataCharacteristics: " + e.getMessage());
        }
    }
    @Override
